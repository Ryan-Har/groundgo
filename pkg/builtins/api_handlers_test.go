...
func TestHandler_handleAPITokenVerify(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "valid token",
			setupContext: func(req *http.Request) *http.Request {
				return req.WithContext(enforcer.ContextWithJWT(req.Context(), validToken))
			},
			mockReturn: func() {
		...
		},
		{
			name: "empty token",
			setupContext: func(req *http.Request) *http.Request {
				return req.WithContext(enforcer.ContextWithJWT(req.Context(), ""))
			},
			mockReturn:     func() {},
			expectedStatus: http.StatusUnauthorized,
		},
		{
			name: "invalid token",
			setupContext: func(req *http.Request) *http.Request {
				return req.WithContext(enforcer.ContextWithJWT(req.Context(), "bad-token"))
			},
			mockReturn: func() {
...
func TestHandler_handleAPILogoutPost(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			name: "invalid token",
			setupContext: func(req *http.Request) *http.Request {
				return req.WithContext(enforcer.ContextWithJWT(req.Context(), "invalid-token"))
			},
			setupMocks: func() {
	...
	tests := []struct {
	...
	}{
		{
			...
			setupContext: func(req *http.Request) *http.Request {
				...
				return req.WithContext(enforcer.ContextWithUser(req.Context(), user))
			},
			expectedStatus: http.StatusOK,
		},
		{
